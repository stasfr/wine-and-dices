export function getErrorMessage(err: unknown, fallback = 'An unexpected error occurred') {
  ...
  if ('statusMessage' in err && typeof err.statusMessage === 'string' && err.statusMessage.length > 0) {
  ...
  if ('statusText' in err && typeof err.statusText === 'string' && err.statusText.length > 0) {
